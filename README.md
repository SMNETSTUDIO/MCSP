...
| 🗀 文件管理器(路径沙箱):在线编辑 + **拖拽/多选上传**(实时进度条)+ **文件下载 / 目录打包 tar.gz 下载** + 重命名、✦ 插件/模组启停(`.jar ⇄ .jar.disabled`,按服务端类型自动认 `plugins/` 或 `mods/`)、◍ 世界管理、◷ 计划任务 | Sandboxed file manager: online editing + **drag-and-drop / multi-file upload** with live progress + **file download / folder download as tar.gz** + rename, plugin/mod toggle (`.jar ⇄ .jar.disabled`, auto-targets `plugins/` or `mods/` per server type), world management, scheduled tasks |
