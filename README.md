...
<details>
<summary><b>🔐 账号与登录</b> <i>Accounts & login</i> — <sub>scrypt · TOTP · OAuth2 · 一次性邀请码 · 强制 2FA</sub></summary>
...
<details>
<summary><b>📦 服务端与实例</b> <i>Servers & instances</i> — <sub>10 种服务端 · 换类型/升降级 · 一键克隆 · 装 Java · 外置登录</sub></summary>
...
<details>
<summary><b>💻 控制台 · 运行 · 网络</b> <i>Console, runtime & networking</i> — <sub>真实 stdout 流 · 崩溃自动重启 · 重启后恢复 · RCON · 6 种内网穿透</sub></summary>
...
| ♻️ **崩溃自动重启**:进程异常退出后自动拉起(默认 5 秒);短时间内连崩多次即停手告警,避免无限重启风暴(默认 10 分钟 3 次,**均可在系统设置里调**)。控制台手动 `stop`、点「停止/强杀」、面板自身关停均不触发 | ♻️ **Crash auto-restart**: relaunches after an abnormal exit (5s by default); gives up with an alert after repeated crashes instead of restart-looping (3 in 10 min by default — **both tunable in system settings**). Manual `stop`, the stop/kill buttons and panel shutdown never trigger it |
...
<details>
<summary><b>📁 文件 · 插件 · 配置</b> <i>Files, plugins & config</i> — <sub>Modrinth 装插件 · 路径沙箱文件管理器 · 世界管理 · 计划任务 · 压缩包</sub></summary>
...
<details>
<summary><b>💾 备份</b> <i>Backups</i> — <sub>tar.gz 全量 · 增量链 · 异地同步 · 恢复前预览</sub></summary>
...
<details>
<summary><b>👥 多租户与配额</b> <i>Multi-tenancy & quotas</i> — <sub>实例隔离 · 三档共享权限 · 内存按堆+堆外计 · CPU 绑核 · 磁盘配额</sub></summary>
...
<details>
<summary><b>⚙ 面板自身</b> <i>The panel itself</i> — <sub>告警推送 · 阈值可配置 · 配置导出导入 · 检查更新 · 双主题</sub></summary>
